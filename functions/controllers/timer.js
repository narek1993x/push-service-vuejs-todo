...
function filterByTime(time, week) {
  ...
  if (typeof week !== "undefined") {
    day = moment().day(capitalize(week)).format("D");
  } else {
    day = moment().format("D");
  }
  ...
  return parseInt(moment(new Date(year, month, day, hours, minutes, sec)).valueOf() / 1000, 10);
}
...
function createNotificationMessage(schedules, subscriptions) {
  return schedules.map((schedule) => {
    const { token } = subscriptions.find((sub) => sub.userId === schedule.ownerId);
    return {
      data: {
        name: schedule.name,
...
async function handleNotificationsTimer() {
  ...
  const filteredByTime = filteredByUserId
    ...
    .sort((a, b) => filterByTime(a.start, a.week) - filterByTime(b.start, b.week));

  const notificationMessages = createNotificationMessage(filteredByTime, subscriptions);
...
const notificationJob = new CronJob("0 */5 * * * *", function () {
